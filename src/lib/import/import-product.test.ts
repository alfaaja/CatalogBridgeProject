...
vi.mock("@/lib/jakmall/fetch", () => ({
  fetchJakMallProductDocument: vi.fn(),
}));
...
describe("JakMall product import orchestration", () => {
  ...
  it("keeps an ordinary timeout message independent from assisted HTML", async () => {
    mockedFetch.mockResolvedValue({
      ok: false,
  ...
  it("imports assisted HTML without fetching or inventing a source identifier", async () => {
    ...
    expect(mockedSave).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        rawSourceData: expect.objectContaining({
          acquisitionMode: "assisted_html",
        }),
        sellingPrice: null,
        sourcePrice: 125000,
    ...
    expect(
      JSON.stringify(mockedSave.mock.calls[0][1].rawSourceData),
